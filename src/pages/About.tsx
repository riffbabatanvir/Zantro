...
export default function About() {
  ...
  useEffect(() => {
    fetch('/api/about')
      .then(r => r.json())
      .then(data => { if (data && typeof data === 'object') setAbout({ ...DEFAULT_ABOUT, ...data }); })
      .catch(() => {});
  }, []);
    ...
    <div className="bg-white dark:bg-neutral-950 text-black dark:text-white">
      ...
      <section className="py-24 md:py-32 px-6 lg:px-12 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
          ...
          <p className="text-lg text-black/50 dark:text-white/50 font-light leading-relaxed">{about.tagline}</p>
        </motion.div>
      </section>
      ...
      <section className="py-24 md:py-32 px-6 lg:px-12 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            ...
            <h3 className="text-3xl md:text-4xl font-black tracking-tighter mb-8">{about.storyTitle}</h3>
            <p className="text-sm text-black/50 dark:text-white/50 font-light leading-relaxed mb-6">{about.storyP1}</p>
            <p className="text-sm text-black/50 dark:text-white/50 font-light leading-relaxed">{about.storyP2}</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {about.values.map((value, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="border-t border-black/10 dark:border-white/10 pt-8">
      ...
      <section className="py-24 md:py-32 px-6 lg:px-12 max-w-7xl mx-auto text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h3 className="text-3xl md:text-5xl font-black tracking-tighter mb-8">{about.ctaTitle}</h3>
          <Link to="/shop" className="inline-flex items-center gap-2 bg-orange-500 text-white px-8 py-4 rounded-full text-sm font-black hover:bg-orange-600 transition-colors shadow-lg">
